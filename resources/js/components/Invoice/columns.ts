...
const getCounterpartyColumn = (role?: string): ColumnDef<Invoice> => {
    ...
    return {
        id: 'counterparty',
        header: () => h('span', { class: headerClass }, isBabysitter ? 'Parent' : 'Babysitter'),
        cell: ({ row }) => {
            const person = (isBabysitter ? row.original.parent : row.original.babysitter) as Person | undefined
